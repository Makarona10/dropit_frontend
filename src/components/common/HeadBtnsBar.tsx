...
const HeadBtnsBar = ({ buttons }: prop) => {
  return (
    <div className="flex flex-wrap items-center pl-2 py-2 gap-1 bg-neutral-800">
      {buttons.map((b: btn) => {
        const Icon = b.ico;
        return (
          <div
            className="flex items-center p-2 sm:px-5 gap-2 rounded-full text-xs md:text-sm opacity-80
